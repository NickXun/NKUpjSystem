    ...
    var sideBarAutoActive=function(){
    	var regx=/\/(table_[a-z]+)[\/|\.]/;
			...
			if($("#"+id).parent().attr('class')=="sub-menu"){
				...
				$("#"+id).parent().parent().children(":first").append("<span class='selected'></span>");     //第一个子节点 ---a，给它追加一个<span class="selected"></span>
			}else{
				//没有子菜单
				$("#"+id).addClass("active");    //给当前li加上active
				$("#"+id).children(":first").append("<span class='selected'></span>");     //第一个子节点 ---a，给它追加一个<span class="selected"></span>
			}
    }
